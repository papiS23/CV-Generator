...
for (let i = 0; i < addBtn.length; i++) { // pętla po ilości przycisków == pętla po sekcjach 
...
function createBar(i) {
  ...
  text.innerText = `${professionValue == undefined ? '' : professionValue}, ${companyValue == undefined ? '' : companyValue} - ${cityValue == undefined ? '' : cityValue} ${selectValue == undefined ? '' : selectValue} ${startValue == undefined ? '' : startValue} ${endValue == undefined ? '' : endValue}`;
